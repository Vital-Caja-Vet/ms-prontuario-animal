...
def listar_animais():
    try:
        animais = (
            Animal.query.options(selectinload(Animal.consultas))
            .filter(or_(Animal.ativo.is_(True), Animal.consultas.any()))
            .all()
        )
...
def buscar_animal(animal_id):
    try:
        ...
        include_inativos = request.args.get('include_inativos', '').strip().lower() in (
            '1', 'true', 't', 'yes', 'y'
        )

        if not animal.ativo and not include_inativos:
            return jsonify({"error": "Animal não encontrado"}), 404
